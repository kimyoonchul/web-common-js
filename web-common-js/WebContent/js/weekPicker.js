...
var setWeekCalendar = function(settingElement,type) {
    var startDate;
    var endDate;
    ...
    $(settingElement).datepicker({
        ...
        onSelect: function(dateText, inst) {
            ...
            if(dateObj.getDay() == 0){
            	...
            	setDay = dateObj.getDay()-1;
            }
            
            if(type == "week"){
            	var ywString = datepickerValue.getFullYear() + '-' + weekNum;
            }else{
            	var ywString = dateAddDel(dateText, -setDay, 'd') + "~" + dateAddDel(dateText, -setDay+6, 'd');
            }
//            dateAddDel(dateText, dateObj.getDay()-1, 'd');
            // 몇주차인지 보고싶을때
            $(this).val(ywString);
...
var convertToWeekPicker = function(targetElement,type) {
...
//        $('<i class="fa fa-calendar showCalendar" aria-hidden="true" style="cursor:pointer;margin-left: 10px;margin-top: 3px;" onclick="javascript:showWeekCalendar(this)"></i>').insertAfter(targetElement);
        setWeekCalendar(targetElement,type);
    } else {
